...
#' The ES in GSVA is similar to GSVA using Kolmogorov-Smirnov (KS) like random walk statistic. Here I illustrate calculating ES in one sample. I set $\tau = 1$.
n_tot = nrow(expr_g)
n_gs = length(gs1)
...
#' #### KS statistics
ks_test_plot(sample, "ID", "rscore", gs1, "gs1 enrichment in sample 1", xlab = "rank score")
#' Similarly one can look at the other two pathways
p2 <- ks_test_plot(sample, "ID", "rscore", gs2, "gs2 enrichment in sample 1", xlab = "rank score")
p3 <- ks_test_plot(sample, "ID", "rscore", gs3, "gs3 enrichment in sample 1", xlab = "rank score")
...
#' #### Classic maximum deviation from 0.
